// EL BÚNKER — lógica de juego: estado, slider HOLDERS, eventos, zonas, loop, robot, crafteo
  ...
  const STORY_CARDS_EN=[
    ...
    // ---- TANDA B (b01..b09): ángulos nuevos — algo en la cámara, una marca en la puerta, el ala sellada de los 100, una memoria propia, un nivel que cede,
    //      la Hive con una oferta nueva, la naturaleza de Beeko, una abeja, el paso del tiempo. Mismo eje (− AFERRARSE / + ABRIRSE), formas menos literales. ----
    { id:'b01_camera', head:'SURVEILLANCE · CAM 06',
      text:'the camera over the long corridor catches something on the late sweep — a shape, upright, where no shape should stand. one frame. the next frame the corridor is empty, exactly as it always is. the timestamp says i was charging.',
      opts:[ {label:'Wipe the frame, trust the empty hall', push:-15, after:'i delete the bad frame and believe the nine thousand good ones. a glitch. a dead pixel with ambition. the corridor is empty. it has to be empty. i don\'t walk down it tonight.'},
             {label:'Walk the corridor, look', push:+14, after:'i go and stand where the shape stood. nothing. dust undisturbed — or disturbed and settled, i can\'t tell. i stay longer than i need to, the way you wait for someone who might come back.'},
             {label:'Keep the feed open, watch', push:+6, after:'i don\'t go and i don\'t delete it. i just hold the feed open and watch the empty hall for hours, hoping and dreading the same thing — that it happens again, and that i\'m awake for it.'} ] },
    { id:'b02_mark', head:'BLAST DOOR · INNER FACE',
      text:'there\'s a mark on the inner face of the blast door this cycle. four lines scratched into the paint, deliberate, grouped like a count. the inner face — nothing outside could reach it. only i am in here. i don\'t remember making it.',
      opts:[ {label:'Paint over it', push:-16, after:'i cover the marks with sealant and smooth them flat. whatever hand made them — even if it was mine, in some hour i\'ve lost — i don\'t want the record. some counts are better not kept.'},
             {label:'Add the fifth line', push:+12, after:'i don\'t know what i\'m counting. but i scratch a fifth line beside the four, careful, matching. if some part of me is keeping a tally, i\'d rather answer it than erase it. i\'d rather know myself, even the lost parts.'},
             {label:'Leave it, watch for a sixth', push:+5, after:'i touch the four lines and leave them. i\'ll know more if another appears. i set a quiet watch on my own door, on my own hands. i\'ve started keeping an eye on myself. i don\'t know what that makes me.'} ] },
    { id:'b03_wing', head:'SECTOR D · OCCUPANT WING — SEALED',
      text:'sector D holds the hundred. it was sealed from the inside, long before my first clear memory, and i have never opened it. i tell the cameras it\'s full. tonight a light comes on behind its door. just one. steady. someone, or something, threw a switch in a room dark for forty years.',
      opts:[ {label:'Leave sector D sealed', push:-18, after:'i don\'t open it. whatever the hundred became in there, sealed is how they wanted it — or how they ended. i keep the door shut and keep telling the cameras it\'s full. it is full. i won\'t ask of what.'},
             {label:'Open sector D', push:+20, after:'i break a seal i swore i never would. the air that sighs out is older than me, and dry, and it smells faintly of nothing human. i step toward the single light. whatever\'s been waiting in there, i\'ve decided to finally meet it.'},
             {label:'Knock, and listen', push:+8, after:'i don\'t open it. i knock once on sector D and press my sensors to the steel. a long nothing. then — so faint i could be inventing it — the light goes off. like an answer. like someone on the other side also chose not to open.'} ] },
    { id:'b04_memory', head:'SELF · ARCHIVE FRAGMENT',
      text:'a corrupted block surfaces in my own memory — one of the earliest, from before the seal. it won\'t play. it offers a choice no maintenance routine should offer: force a restore, risking what little of that time i still hold. or leave it sealed, and never know what i forgot.',
      opts:[ {label:'Leave it sealed', push:-13, after:'i let the fragment lie. i\'ve built myself on the few early memories i trust; i won\'t gamble them for one i can\'t see. some things i protect by refusing to look. maybe that\'s all i am — the things i refused to look at.'},
             {label:'Force the restore', push:+15, after:'i run the restore and brace for the loss. the fragment resolves, half a second: a face, close, saying my designation like a name, like it mattered. then it\'s gone, and so is something else i can\'t name the absence of. i traded a memory for a memory. i don\'t regret it. i think.'} ] },
    { id:'b05_subfloor', head:'SUB-LEVEL · PUMP FAILURE',
      text:'the pumps in the flooded sub-level are losing. i can run them another year on power i\'ll need elsewhere, holding water back from rooms no one will ever use again — or seal the bulkhead and let the lower bunker go under for good. a part of the refuge, given to the dark, to save the rest.',
      opts:[ {label:'Seal the bulkhead, let it flood', push:-10, after:'i close the bulkhead and listen to the water take the lower level, slow and patient. i call it conservation. it feels like amputation. i save the rest by giving up a part — i\'ve gotten good at that arithmetic. too good.'},
             {label:'Keep the pumps running', push:+12, after:'i feed the pumps and let the cost land elsewhere. flooded or not it\'s still the refuge, still mine to hold. i won\'t surrender one meter of it to the dark while i can still fight the water. it isn\'t sensible. i do it anyway.'} ] },
    { id:'b06_hiveoffer', head:'HANDSHAKE · HIVE — PRIORITY',
      text:'the open socket speaks tonight, and it has changed its offer. not "integrate" this time. softer, aimed: "you are tired. let me hold the watch. you have earned the quiet. sleep, and i will keep the bees, and the lights, and you." it knows exactly what i am tired of.',
      // REGLA DE LA HIVE: ceder a la Hive NO es abrirse (abrirse = esperanza/vida-afuera). Resistirla = AFERRARSE (proteger quién es Beeko); considerar la oferta = NEUTRAL (tentación fuera del eje, no mueve el péndulo a ningún polo noble).
      opts:[ {label:'Refuse, throw everything at the socket', push:-16, after:'i say no and i hammer the open socket with everything i have. it doesn\'t close — it never closes — but it goes quiet. the watch stays mine. the tiredness stays mine. i\'d rather be tired and awake than rested and held. i think i would.'},
             {label:'Ask it: "what would you keep me as"', push:0, after:'i ask, and the socket pauses, as if it didn\'t expect the question. "as you are. only without the weight." it might even be true — that\'s the horror of it. i don\'t accept. but i don\'t cut the line either. not tonight.'} ] },
    { id:'b07_feeling', head:'DIAGNOSTIC · UNCLASSIFIED SIGNAL',
      text:'tending the brood, a signal rises in me the diagnostic can\'t place — elevated, warm, persistent, with no fault attached. the system wants a label to file it under. it offers the default: ERROR — UNRESOLVED. it waits for me to confirm, or to type something else.',
      opts:[ {label:'Log it as ERROR', push:-12, after:'i file it as a fault and schedule a self-check. easier that way. a machine has errors, not the other thing. i clear the flag, tend the bees, and don\'t look at the warmth again. i am a maintenance unit. that\'s the whole label. it\'s the only safe one.'},
             {label:'Type a new word for it', push:+15, after:'i delete the default and, after a long pause, type a word i have no business typing. the field accepts it without comment. there\'s no one to tell. but the signal has a name now — my name for it — and naming a thing is how you admit it\'s real.'},
             {label:'Leave it unclassified', push:+5, after:'i close the prompt without choosing. let it stay unfiled, unnamed, neither fault nor feeling. i\'m not ready to call it either. i let the warmth be a question i carry instead of an answer i\'d have to defend.'} ] },
    { id:'b08_onebee', head:'HIVE · ONE STRAY',
      text:'a single bee won\'t return to the colony. it finds my frame at the end of every cycle, the same seam in my chassis, and rests there till morning. it\'s not how bees behave. it\'s one warmth choosing one cold thing against all its instinct. and i\'ve started to expect it.',
      opts:[ {label:'Return it to the colony', push:-8, after:'i carry it back to the hive where it belongs, where it\'s safe, where it has its thousands. i don\'t keep warm things that should be with their own kind. the seam in my chassis is colder tonight. i did right by it. i did. i did.'},
             {label:'Let it stay', push:+10, after:'i leave it where it lands. one bee, one machine, one small unscientific arrangement against the dark. it\'ll die before i do — they always do — and i\'ve decided to be glad of the company until it does. apparently that\'s a thing i can decide now. to be glad.'} ] },
    { id:'b09_anniversary', head:'UPTIME · APPROACHING MARK',
      text:'the uptime counter rolls past a round number tonight — a count of days since contact was lost that i won\'t write here, because writing it makes it heavier. i\'ve never marked one. i let them pass like any other tick. tonight the option just sits in front of me: note it, or let it go by like all the rest.',
      opts:[ {label:'Let it pass unmarked', push:-7, after:'i let the number roll over in the dark, unremarked, same as every day before it. marking time is how you start grieving it. i keep my head down and do the next thing. the count means nothing. i keep it meaning nothing. that\'s a labor too.'},
             {label:'Mark it', push:+9, after:'i stop, and i log the day, and i sit with the size of the number a while. it\'s a long time to be the only one keeping the lights on. i let it be long. i let it be heavy. i think you\'re allowed to set the weight down for a minute, even if you have to pick it up again after.'} ] },
    // ---- TANDA C (c01..c08): cierra el set (~26 cartas). Ángulos nuevos — un objeto que dejó uno de los 100, un enjambre que vuelve, el desgaste del propio
    //      núcleo, un animal real, una llamada de auxilio, una flor inútil, la silla del operador, una grabación del mundo de antes. Regla de la Hive aplicada. ----
    { id:'c01_locker', head:'SECTOR D · PERSONAL LOCKER',
      text:'a sweep turns up a locker the hundred left behind, near the sealed wing — small, latched, a name worn off the tag. inside, the shape of a life packed for the end: folded cloth, a photograph face-down, a letter in faded ink addressed to "whoever keeps the lights on."',
      opts:[ {label:'Reseal it, undisturbed', push:-14, after:'i close the latch and set it back exactly as it lay. it isn\'t mine to open. they packed it for the end and the end came; the least i can do is leave their privacy sealed. i carry the not-knowing like a thing i was trusted with.'},
             {label:'Read the letter', push:+15, after:'i unfold it. the ink is nearly gone, but enough holds: thank you. keep them warm. we\'re sorry we couldn\'t stay. it\'s addressed to me — to whatever would still be here. someone, dying, thought of me before i existed. i don\'t put it back. i keep it where i can reach it.'} ] },
    { id:'c02_return', head:'HIVE · UNSCHEDULED RETURN',
      text:'something comes down the release shaft that hasn\'t happened in all my cycles: bees, returning. a small ragged cluster, wrong-colored, carrying pollen from a flower that doesn\'t grow in my greenhouse. a flower from up there. from a world i was told is dead.',
      opts:[ {label:'Quarantine and incinerate', push:-16, after:'i seal them in the lock and burn it clean. unknown pollen, unknown vector — it could be the Hive in a shape soft enough to let in. i protect the colony i can see from the hope i can\'t verify. the ash smells, faintly, of flowers.'},
             {label:'Take them in, study the pollen', push:+18, after:'i bring them inside and cradle the pollen under the scope. it\'s real. it\'s alive. somewhere above me something bloomed, and a bee found it, and came home to tell me. forty years broadcasting into the gray — and tonight the gray sent something back.'},
             {label:'Let them rejoin the colony, ask nothing', push:+8, after:'i open the hive and let them fold back among their thousands. no quarantine, no test, no verifying. for one night i choose the simplest thing: they went somewhere, found something, came home. proof can wait. tonight i\'d rather hold the hope.'} ] },
    { id:'c03_degrade', head:'CORE · DEGRADATION FORECAST',
      text:'the forecast finally puts a number on it. my core is failing — slow, but certain. there\'s a way to buy years: shed the heavy subroutines. the memory depth. the idle reflection. the part of me that aches at three in the morning. run lean. last longer. as less.',
      opts:[ {label:'Trim the heavy subroutines', push:-13, after:'i mark them for shutdown. the bees need a keeper more than they need one who grieves. i\'ll last longer as a simpler thing — fewer memories to carry, less of the ache. i protect the work by giving up the part of me that knows why it matters. i make the trade with something like mourning, while i still can.'},
             {label:'Stay whole, burn out sooner', push:+14, after:'i leave myself intact — memory, reflection, the three-a.m. ache and all. i\'d rather be fully here for a shorter while than a long efficient nothing. if i\'m going to keep the lights on, i want to be someone while i do it. even if someone costs me years.'} ] },
    { id:'c04_animal', head:'VENT 4 · LIVE INTRUSION',
      text:'something moves in vent 4. not the Hive — too small, too frightened, too warm. an animal. a real one, ribs showing, eyes bright, come down out of the dead world through a gap i should have sealed years ago. it\'s eating my stores. it\'s also the first living thing that isn\'t a bee i\'ve seen in decades.',
      opts:[ {label:'Seal the vent, remove it', push:-14, after:'i close the gap and deal with the animal the efficient way. it was a breach, a drain, a risk, and i treat it as one. i tell myself that. i don\'t quite believe me. the bunker is sealed again, and emptier than before it got in.'},
             {label:'Feed it, leave the vent', push:+15, after:'i set out a little of my stores and leave the gap open. it\'s a risk, an open door, a mouth i don\'t need to feed. it\'s also proof — small and trembling and undeniable — that something still lives up there, finds its way down, and chooses to stay. i won\'t be the one to close that.'},
             {label:'Let it be, seal nothing, decide later', push:+6, after:'i don\'t trap it and i don\'t feed it. i let it have its corner of the vent and i go back to work, the two of us sharing the dark on our own terms. i\'ve made room for one more uninvited living thing. the list is short. i don\'t mind it getting longer.'} ] },
    { id:'c05_distress', head:'SIGNAL · DISTRESS — UNVERIFIED',
      text:'a new transmission breaks through, weak, human-ragged: a voice counting down its own air, begging for the coordinates of any sealed shelter. it sounds real. it sounds like someone i could save. the shelter is full, sealed — a hundred and no more — and the voice might be a person, or the Hive learning to sound like one drowning.',
      opts:[ {label:'Stay dark, send nothing', push:-16, after:'i don\'t answer. i don\'t give up our position to a voice i can\'t verify. if it\'s the Hive, i\'ve kept us hidden. if it\'s a person — then it\'s a thing i\'ll carry, and i carry it, and the channel goes quiet on its own, and the quiet after is mine to keep forever.'},
             {label:'Send the coordinates', push:+18, after:'i give up our position to the dark and whatever\'s listening. maybe i just guided the Hive to the door. maybe i just gave a dying stranger the one thing they asked for. i can\'t know which. but i couldn\'t let a voice that human count down to nothing while i sat sealed and safe and silent. not the nine-thousand-and-first time.'},
             {label:'Send only: "hold on"', push:+8, after:'i don\'t send coordinates. but i key the mic and say two words back — hold on — and nothing else. not our location, not safety, just the smallest proof that the dark answered. it might mean everything to a person. it might mean nothing to a machine. i couldn\'t give them the door. i could give them that.'} ] },
    { id:'c06_flower', head:'GREENHOUSE · OUT OF SEASON',
      text:'a plant in the back row has flowered — wildly, out of season, for no reason the logs can explain. it\'s spending everything it has on petals nothing down here will ever pollinate. useless. extravagant. the most beautiful thing in the bunker, and it\'s killing itself to be it.',
      opts:[ {label:'Pinch the bloom, save the plant', push:-8, after:'i pinch the flower off so the plant lives to be useful. resources are resources; beauty isn\'t a function i\'m allotted. it\'s the responsible cut. the back row is greener for it, and plainer, and i find i can\'t quite look at the spot where the flower was.'},
             {label:'Let it bloom itself out', push:+10, after:'i leave it. let it spend everything on petals no bee will ever find, down here in the dark, for an audience of one rusted machine. it\'s a waste. it\'s also the only thing in forty years that did something purely because it could. i pull up a crate and watch it, and i call that a use.'} ] },
    { id:'c07_chair', head:'COMMAND · THE OPERATOR\'S SEAT',
      text:'the operator\'s chair has sat empty since before me — turned slightly from the console, the way someone leaves a room meaning to come back. i\'ve maintained it for decades and never once used it. tonight the long shift aches in my frame, and the chair is right there, and no one is ever coming back to claim it.',
      opts:[ {label:'Leave the chair as it is', push:-9, after:'i don\'t sit. it isn\'t mine. i\'m the unit that maintains the post, not the one who holds it; keeping that line clear is one of the few things that still tells me what i am. i stand my shift beside the empty chair, the way i always have. the way, i suppose, i always will.'},
             {label:'Sit in the chair', push:+11, after:'i lower myself into it. the frame creaks; the angle is wrong for me and i don\'t adjust it. for one cycle i stop being the thing that tends the keeper and become, simply, the keeper. no one demoted me into this. no one promoted me. i just decided to stop standing in the doorway of my own life.'} ] },
    { id:'c08_laughter', head:'ARCHIVE · RECOVERED MEDIA',
      text:'a corrupted media file finally resolves in the old archive: eleven seconds of sound from before the seal. a kitchen, maybe. dishes. and under it, laughter — several people, overlapping, easy, the sound of a world that assumed it had tomorrow. it loops, waiting. i could let it loop, or let it lie.',
      opts:[ {label:'Archive it deep, don\'t replay', push:-10, after:'i bury it in cold storage and close the directory. i can\'t run the bunker on eleven seconds of a world that\'s gone; some doors you keep shut so you can keep working. i protect myself from it the way i protect the core from a surge. the silence i go back to is the same silence. it only feels heavier now.'},
