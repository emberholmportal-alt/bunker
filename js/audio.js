// EL BÚNKER — audio (WebAudio)
  ...
  function flap(n){if(!audioOn||!actx)return;const t=actx.currentTime;const s=actx.createBufferSource();s.buffer=noiseBuf;const bp=actx.createBiquadFilter();bp.type='bandpass';bp.frequency.value=2100+Math.random()*700+Math.min(n||1,6)*40;bp.Q.value=1.1;const g=actx.createGain();const vol=Math.min(.14,.06+(n||1)*.012);g.gain.setValueAtTime(0,t);g.gain.linearRampToValueAtTime(vol,t+.004);g.gain.exponentialRampToValueAtTime(.0006,t+.06);s.connect(bp);bp.connect(g);g.connect(master);s.start(t);s.stop(t+.07);}
